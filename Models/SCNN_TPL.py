...
class SCNN_TPL(nn.Module):
    ...
    def __init__(self, apriori_relevance, leak_rate, dropout_rate, input_channels):
        ...
        self.core1 = nn.Conv1d(in_channels=input_channels, out_channels=8, kernel_size=15, stride=50)
        ...
        self.core5 = nn.Linear(24768, 32) #25152
    ...
    def forward(self, x):
        ...
        x_core = self.core2_2(x_core)
        x_core = self.core_norm2_2(x_core)
        x_core = self.dropout(x_core)
        x_core = self.reLU(x_core)
        ...
        x_core = self.dropout(x_core)
        x_core = self.tanh(x_core)
        x_core = self.core3_2(x_core)
        x_core = self.core_norm3_2(x_core)
        x_core = self.dropout(x_core)
        x_core = self.reLU(x_core)
