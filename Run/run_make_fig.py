...
def display_and_save_figures(sweep_id, threshold, training_fold_number, val_fold_number, number_of_training_folds, validation_test_folds, display=False):
    ...
    for eval_mode in ["train", "val", "test"]:
        ...
        figures_base_dir = "/local/scratch/code/TROPHY/colon_data_analysis/CRIME/Figures"
        sweep_figures_dir = os.path.join(figures_base_dir, f"{sweep_id}_{threshold}")

        # Create directory for the sweep (and the Figure directory if necessary)
        os.makedirs(figures_base_dir, exist_ok=True)
        os.makedirs(sweep_figures_dir, exist_ok=True)
        if not os.path.exists(os.path.join(sweep_figures_dir, f"total_eval_stats_{eval_mode}.npy")):
            np.save(os.path.join(sweep_figures_dir, f"total_eval_stats_{eval_mode}.npy"), total_eval_stats)
        ...
        for i, sample_of_interest in enumerate(samples_of_interest):
            ...
            data_figure_path = os.path.join(sweep_figures_dir, 
                                        f"sample_{sample_of_interest}_{eval_mode}_data.png")
            pred_figure_path = os.path.join(sweep_figures_dir, 
...
display_and_save_figures("vlu8blib", 0.55, 0, 0, 2, 2)
display_and_save_figures("lmtqrjoj", 0.5, 0, 0, 2, 2) 
display_and_save_figures("lmtqrjoj", 0.55, 0, 0, 2, 2)
